...
function saveMessageToStorage(message) {
  // Skip welcome messages entirely - we'll manage them separately
...
socket.on('message', messageData => {
  ...
  if (messageData.includes("Welcome to the room:")) {
    // Only display the welcome message on the very first load
    // after page refresh, not when loading history
    if (isFirstLoad) {
      displayMessage(messageData, false); // Don't save welcome messages
      isFirstLoad = false;
    }
  } else {
    // For all other messages, display and save normally
